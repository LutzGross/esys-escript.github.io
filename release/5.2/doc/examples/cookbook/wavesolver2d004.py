...
u=x*0.
#u=abs(U0*(cos(length(x-xc)*3.1415/src_radius)+1)*whereNegative(length(x-xc)-src_radius)*dunit)
#u=whereNegative(length(x-xc)-src_radius)*dunit

#maxi=max(u.toListOfTuples())
...
#    t=1.
    ...
    ## TIMED SOURCE
    srcf=np.exp(-50.*t)*np.sin(40*3.14157*t)
    setValueOfDataPoint(srcf,xc,u)
    ##OLD WAY
    g=grad(u)
    break
