...
\autoref{eqn:poisson}. There are only two non-zero coefficients, $A$ and $Y$,
...
of the potential is goverened by an inverse-square distance law. Thus, in the
...
migration from a 2D to a 3D problem almost trivial.
...
solution. The profiles in \auotref{fig:ex10p} further demonstrates the affect
...
appropriately dimensioned model for the problem. 
...
large for the boundary conditions to be satisfied. This can be problematic when
trying to solve problems that require a dense mesh, either for solution
resolution of stability reasons. The cost of solving for vast numbers of
elements can be prohibitive.
...
This example creates a mesh which contains a high resolution sub-domain in its
...
\end{python}
The two sub-domains now have a common geometry and no overlaping features as
