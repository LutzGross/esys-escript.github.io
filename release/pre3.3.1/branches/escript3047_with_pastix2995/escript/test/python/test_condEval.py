...
__copyright__="""Copyright (c) 2010 by University of Queensland
Earth Systems Science Computational Center (ESSCC)
http://www.uq.edu.au/esscc
Primary Business: Queensland, Australia"""
...
class Test_CondEval(unittest.TestCase):
   ...
   def test_Constant(self):
	...
	self.failUnless(Lsup(condEval(m1,d1,d2)-(1,3))<self.RES_TOL)
	self.failUnless(Lsup(condEval(mm1,d1,d2)-(2,4))<self.RES_TOL)
	#Now we try lazy
	d1=delay(d1)
	d2=delay(d2)
	m1=delay(m1)
   ...
   def test_Tagged(self):
	...
	self.failUnless(Lsup(z-res)<self.RES_TOL)
	...
	y=z-res
	Lsup(y)
	self.failUnless(Lsup(condEval(mt1,t1,t2)-res)<self.RES_TOL)
	res=Data((2,4,6), self.x.getFunctionSpace())
	res.setTaggedValue(1,(-1,-1,-1))
	res.setTaggedValue(4,(2,4,6))
	self.failUnless(Lsup(condEval(mt2,t1,t2)-res)<self.RES_TOL)
   ...
   def test_Expanded(self):
	...
	self.failUnless(Lsup(condEval(me1, e1,e2)+condEval(me2, e1,e2)-(3,7,11))<self.RES_TOL)
	...
	self.failUnless(Lsup(condEval(ml1, le1,le2)+condEval(ml2, le1,le2)-(3,7,11))<self.RES_TOL)
   ...
   def test_Errors(self):
	...
	#Non-scalar mask
	self.failUnlessRaises(RuntimeError,condEval, mS1,d1,d2)
	#shape mismatch
	self.failUnlessRaises(RuntimeError,condEval, m, d1, d3)
   ...
   def test_promote(self):	
	...
	for v in [False, True]:
		...
		self.failUnless(Lsup(condEval(mt1,d1,d2)-(1,3))<self.RES_TOL)
		...
		d2.tag()
		self.failUnless(Lsup(condEval(me1,d1,d2)-(1,3))<self.RES_TOL)
		...
		if v:
		    d2=delay(d2)
		self.failUnless(Lsup(condEval(mc1,d1,d2)-(7,19))<self.RES_TOL)	
