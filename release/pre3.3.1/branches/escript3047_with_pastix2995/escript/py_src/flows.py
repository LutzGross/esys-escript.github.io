...
class DarcyFlow(object):
    ...
    *u_{i,i} = f*
    ...
    def __init__(self, domain, weight=None, useReduced=False, adaptSubTolerance=True):
        ...
        :type domain: `Domain`
        :param weight: the weighting factor for the incempressiblity equation *u_{i,i} = f* in the variational principle. 
        If not present an apprppriate weight is chosen.
        :type  weight: positive sacalar
	...
	:type adaptSubTolerance: ``bool``	
        ...
        if weight == None:
           self.__l=None
           self.__update_weight=True
        else:
           self.__update_weight=False
           self.__l=weight
	...
	*(Q^* S Q)p=Q^*G*
         ...
         as a preconditioner where S=weight*k^2/dx^2
	...
	*(Q^* S Q)p=Q^*G*
        ...
        was a preconditioner here S=weight*k^2/dx^2
    ...
    def setValue(self,f=None, g=None, location_of_fixed_pressure=None, location_of_fixed_flux=None, permeability=None):
        ...
        if permeability!=None:
           ...
           if perm.getRank()==0:
               ...
               perm2=perm
               perm=Tensor(0.,self.__pde_p.getFunctionSpaceForCoefficient("A"))
               perm_inv=Tensor(0.,self.__pde_p.getFunctionSpaceForCoefficient("A"))
           ...
           if self.__update_weight: self.__l = util.longestEdge(self.domain)**2/3.14**2*util.length(self.__permeability_inv)
           self.__pde_v.setValue(D=self.__permeability_inv, A=self.__l*util.outer(util.kronecker(self.domain),util.kronecker(self.domain)))
           s=self.__pde_p.getFunctionSpaceForCoefficient("A").getSize()
           self.__pde_p.setValue(A=util.transposed_tensor_mult(self.__permeability,self.__permeability)*self.__l/s**2)
	...
	self.setSubProblemTolerance()
        self.__pde_v.setValue(X=util.sqrt(self.__l)*self.__f*util.kronecker(self.domain), r=fixed_flux)
        g2=util.tensor_mult(self.__permeability_inv,self.__g)
        if p == None:
           self.__pde_v.setValue(Y=g2)
        else:
           self.__pde_v.setValue(Y=g2-util.grad(p))
        return self.__pde_v.getSolution()

    def __Aprod(self,dp):
          if self.getSolverOptionsFlux().isVerbose(): print "DarcyFlux: Applying operator"
          Gdp=util.grad(dp)
          self.__pde_v.setValue(Y=-Gdp,X=Data(), r=Data())
    ...
    def __Msolve_PCG(self,r):
	  if self.getSolverOptionsPressure().isVerbose(): print "DarcyFlux: Applying preconditioner"
          self.__pde_p.setValue(X=r[0]-r[1], Y=Data(), r=Data())
    ...
    def solve(self,u0,p0, max_iter=100, verbose=False, max_num_corrections=10):
         ...
         *(K^[-1]+D^* weight D)u+G p=D^* sqrt(weight) f + K^[-1]g*
         *G^*u+G^* K Gp=G^*g*

         where *D* is the *div* operator and *(Gp)_i=p_{,i}* for the permeability *K=k_{ij}*.
         We eliminate the flux form the problem by setting

         *u=(K^[-1]+D^* weight D)^{-1}(D^* sqrt(weight) f + K^[-1]g* -G p ) with u=u0 on location_of_fixed_flux

         form the first equation. Inserted into the second equation we get

         *G^*(K-(K^[-1]+D^* weight D)^{-1})Gp= G^*(g-(K^[-1]+D^* weight D)^{-1}(D^* sqrt(weight) f + K^[-1]g))* with p=p0  on location_of_fixed_pressure

         which is solved using the PCG method (precondition is *G^*K^2*weight*l^2/dx^2 G*. In each iteration step
         PDEs with operator *K^[-1]+D^* weight D* and with *G^*K^2*weight*l^2/dx^2 G* needs to be solved using a sub iteration scheme.
         """
	 ...
	 self.setSubProblemTolerance()
         ...
         while not converged:
               ...
               norm_v=util.integrate(util.inner(v,util.tensor_mult(self.__permeability_inv,v)))**0.5
               norm_Gp=util.integrate(util.inner(Gp,KGp))**0.5
               if self.verbose:
                   print "DarcyFlux: L2: g-v-K*grad(p) = %e."%(util.integrate(util.length(self.__g-util.interpolate(v,Function(self.domain))-KGp)**2)**(0.5),)
                   print "DarcyFlux: L2: f-div(v) = %e."%(util.integrate((self.__f-util.div(v))**2)**(0.5),)
                   print "DarcyFlux: K^{-1}-norm of v = %e."%norm_v
                   print "DarcyFlux: K-norm of grad(p) = %e."%norm_Gp
               if norm_v == 0.: 
                  if norm_Gp == 0.: 
                     return v,p
                  else:
                  ...
                  if norm_Gp == 0.: 
                    ...
                    fac=2./(1./norm_v+1./norm_Gp)
               #fac=max(norm_Gp, norm_v)
               ATOL=atol+rtol*fac
               if self.verbose: print "DarcyFlux: absolute tolerance ATOL = %e (fac= %e)."%(ATOL,fac)
               if norm_r == None or norm_r>ATOL:
                   ...
                   # initial residual is r=G^*(self.__g-KGp - v) 
                   p,r, norm_r=PCG(ArithmeticTuple(self.__g-KGp,v),
                                   self.__Aprod,
                                   p,
                                   self.__Msolve_PCG,
                                   self.__inner_PCG,
    ...
    def setTolerance(self,rtol=1e-4):
        ...
        *|g-v-K gard(p)|_PCG <= atol + rtol * (1/|K^{-1/2}u|_0 +1/|K^{1/2}grad(p)|_0)^(-1}*
