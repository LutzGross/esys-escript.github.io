    ...
    if (escript::isDirectSolver(sb.getSolverMethod())) {
        ...
        if (problem.is_null()) {
            ...
            if (!prec.is_null()) {
                ...
                // For complex types, SO_DEFAULT also resolves to BICGSTAB.
                const auto m = sb.getSolverMethod();
                const bool needsRightPrec = (m == escript::SO_METHOD_BICGSTAB)
                    || (std::is_same_v<ST, cplx_t> && m == escript::SO_DEFAULT);
                if (needsRightPrec)
...
//#ifdef ESYS_TRILINOS_14
    ...
    for (LO lrb = 0; lrb < static_cast<LO>(mat.getLocalNumRows()); lrb++) {
        ...
        const LO numIndices = static_cast<LO>(indices.extent(0));
//#else
