...
void Assemble_PDE_Single_3D(const AssembleParameters& p, const ElementFile* elements,
...
#pragma omp for
            for (index_t e = 0; e < elements->numElements; e++) {
                if (elements->Color[e] == color) {
                    ...
                    if (add_EM_F)
                        util::addScatter(p.numShapes, &row_index[0], p.numEqu, &EM_F[0], F_p, p.row_DOF_UpperBound);
                    if (add_EM_S)
