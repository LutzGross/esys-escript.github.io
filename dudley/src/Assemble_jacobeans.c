                           ...
                           double* dTdX, double* volume, index_t* element_id)
{
    ...
    const double quadweight=(numQuad==1)?1./6:1./24;
    const double DTDV[4][3]={{-1, -1, -1}, {1, 0, 0}, {0, 1, 0}, {0, 0, 1}};
    #pragma omp parallel 
    {
	...
	for(e=0;e<numElements;e++)
	{
		...
		dXdv02+=X0_loc*DTDV[s][2];
		dXdv12+=X1_loc*DTDV[s][2];
		dXdv22+=X2_loc*DTDV[s][2];
	    }
		...
		for (q=0;q<numQuad;q++)
		{
		    for (s=0;s<numTest; s++)
		    {
			dTdX[INDEX4(s,0,q,e,numTest,DIM,numQuad)]=DTDV[s][0]*dvdX00+DTDV[s][1]*dvdX10+DTDV[s][2]*dvdX20;
			dTdX[INDEX4(s,1,q,e,numTest,DIM,numQuad)]=DTDV[s][0]*dvdX01+DTDV[s][1]*dvdX11+DTDV[s][2]*dvdX21;
			dTdX[INDEX4(s,2,q,e,numTest,DIM,numQuad)]=DTDV[s][0]*dvdX02+DTDV[s][1]*dvdX12+DTDV[s][2]*dvdX22;
		    }
...
/*                                                            */
void Assemble_jacobeans_3D_M2D_E2D(double* coordinates, dim_t numQuad, dim_t numElements, dim_t numNodes, index_t* nodes,
                                   double* dTdX, double* volume, index_t* element_id)
{
    #define DIM 3
    #define LOCDIM 2
    ...
    char error_msg[LenErrorMsg_MAX];
    // numQuad is 1 or 3
    const double quadweight=(numQuad==1)?1./2:1./6; 
