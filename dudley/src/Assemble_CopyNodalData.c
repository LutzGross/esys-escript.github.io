...
void Dudley_Assemble_CopyNodalData(Dudley_NodeFile* nodes,escriptDataC* out,escriptDataC* in) {
    ...
    if (numComps!=getDataPointSize(in)) {
       Dudley_setError(TYPE_ERROR,"Dudley_Assemble_CopyNodalData: number of components of input and output Data do not match.");
    ...
    if (in_data_type == DUDLEY_NODES) {
        if (! numSamplesEqual(in,1,Dudley_NodeFile_getNumNodes(nodes))) {
               Dudley_setError(TYPE_ERROR,"Dudley_Assemble_CopyNodalData: illegal number of samples of input Data object");
       }
       ...
       if ( (((out_data_type == DUDLEY_NODES) || (out_data_type == DUDLEY_DEGREES_OF_FREEDOM)) && !isExpanded(in) && (mpiSize>1))) {

               Dudley_setError(TYPE_ERROR,"Dudley_Assemble_CopyNodalData: DUDLEY_DEGREES_OF_FREEDOM to DUDLEY_NODES or DUDLEY_DEGREES_OF_FREEDOM requires expanded input data on more than one processor.");
       }
       ...
       if ( (out_data_type == DUDLEY_DEGREES_OF_FREEDOM) && !isExpanded(in) && (mpiSize>1)) {
               ...
               Dudley_setError(TYPE_ERROR,"Dudley_Assemble_CopyNodalData: DUDLEY_REDUCED_DEGREES_OF_FREEDOM to DUDLEY_DEGREES_OF_FREEDOM requires expanded input data on more than one processor.");
       }
    } else {
    ...
    if (out_data_type == DUDLEY_NODES) {
        if (! numSamplesEqual(out,1,Dudley_NodeFile_getNumNodes(nodes))) {
               Dudley_setError(TYPE_ERROR,"Dudley_Assemble_CopyNodalData: illegal number of samples of output Data object");
       }
	   ...
	   requireWrite(out);
	      ...
	      #pragma omp parallel private(n)
	      {
        	#pragma omp for schedule(static)
             ...
             Dudley_setError(TYPE_ERROR,"Dudley_Assemble_CopyNodalData: cannot copy from reduced nodes to degrees of freedom.");
           } else if (out_data_type == DUDLEY_REDUCED_DEGREES_OF_FREEDOM) {
	      int nComps = Paso_Distribution_getMyNumComponents(nodes->reducedDegreesOfFreedomDistribution);
        ...
        /*********************** DUDLEY_REDUCED_DEGREES_OF_FREEDOM **************************************************/
           ...
           if  (out_data_type == DUDLEY_NODES) {
             Dudley_setError(TYPE_ERROR,"Dudley_Assemble_CopyNodalData: cannot copy from reduced degrees of freedom to nodes.");
           } else if (out_data_type == DUDLEY_REDUCED_NODES) {
		...
		#pragma omp for schedule(static)
             ...
             Dudley_setError(TYPE_ERROR,"Dudley_Assemble_CopyNodalData: cannot copy from reduced degrees of freedom to degrees of freedom.");
           }
        }
   }
   return;
