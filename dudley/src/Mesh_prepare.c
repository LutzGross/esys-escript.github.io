...
void Dudley_Mesh_prepare(Dudley_Mesh* in, bool_t optimize) {
     ...
     if (! (Dudley_checkPtr(distribution) || Dudley_checkPtr(node_distribution))) {
        ...
        if (Dudley_noError()) Dudley_Mesh_distributeByRankOfDOF(in,distribution);
     }
     ...
     if (Dudley_noError()) Dudley_Mesh_optimizeElementOrdering(in);
...
*/
          ...
          if (Dudley_noError()) Dudley_Mesh_createNodeFileMappings(in,numReducedNodes,indexReducedNodes,distribution, node_distribution);
        }
...
{
    if (Dudley_noError()) Dudley_NodeFile_setTagsInUse(in->Nodes);
