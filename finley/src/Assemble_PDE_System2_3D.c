...
void  Finley_Assemble_PDE_System2_3D(Assemble_Parameters p, Finley_ElementFile* elements,
    ...
    {
       double EM_S[len_EM_S], EM_F[len_EM_F];
       ...
       double *Vol, *DSDX, *A_p, *B_p, *C_p, *D_p, *X_p, *Y_p;
                   ...
                   if (extendedA) {
                      ...
                      for (s=0;s<p.row_NS;s++) {
                        for (r=0;r<p.col_NS;r++) {
                            ...
                            for (k=0;k<p.numEqu;k++) {
                               for (m=0;m<p.numComp;m++) {
                                  EM_S[INDEX4(k,m,s,r,p.numEqu,p.numComp,p.row_NN)]+= rtmp00*A_p[INDEX4(k,0,m,0,p.numEqu,DIM,p.numComp)]
                            ...
                            for (m=0;m<p.numComp;m++) {
                              ...
                              for (q=0;q<p.numQuad;q++) {
                                   rtmp+=Vol[q]*S[INDEX2(s,q,p.row_NS)]*(
                                             ...
                                             +C_p[INDEX4(k,m,2,q,p.numEqu,p.numComp,DIM)]*DSDX[INDEX3(r,2,q,p.row_NN,DIM)]);
                              }
                              EM_S[INDEX4(k,m,s,r,p.numEqu,p.numComp,p.row_NN)]+=rtmp;
                            }
                          }
