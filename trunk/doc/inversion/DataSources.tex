...
The parameter \member{datatype} must be one of
...
file, \member{datafile} specifies the name of the data file and
...
the header file except for the file extension. The \member{altitude} parameter
...
The parameter \member{datatype} must be one of
\class{DataSource}.\member{GRAVITY} or \class{DataSource}.\member{MAGNETIC}
depending on the type of data, \member{filename} is the name of the file and
\member{altitude} specifies the altitude in meters of the measurements.
The \member{altitude} parameter is only used with 3-dimensional domains and
