...
$\alpha$ defines a scaling factor which is typically set to one.
...
\begin{classdesc}{GravityModel}{domain, w, g
        \optional{, useSphericalCoordinates=False}
        \optional{, scale=1}
        \optional{, tol=1e-8}}
opens a gravity forward model over the \member{domain} with weighting factors
\member{w} ($=\omega^{(s)}$) and measured gravity acceleration
anomalies \member{g} ($=g^{(s)}$).
The weighting factors and the measured gravity acceleration anomalies must be
vectors.
If \member{useSphericalCoordinates} is \True spherical coordinates are used.
\member{scale} sets the scaling factor $\alpha$.
\member{tol} sets the tolerance for the solution of the PDE~(\ref{ref:GRAV:EQU:100}).
