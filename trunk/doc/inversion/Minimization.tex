...
In some application cases the optimization problem to be solved defines a quadratic programming problem which would allow
...
\begin{classdesc}{MinimizerLBFGS}{\optional{J=\None}}
Constructs an instance of the limited-memory Broyden-Fletcher-Goldfarb-Shanno \emph{L-BFGS} solver.
\member{J} set the cost function to be minimized, see Section~\ref{chapter:ref:Minimization: costfunction class}. 
If not present the cost function needs to be set using the \member{setCostFunction} method.
\member{imax} sets the maximum number of iteration steps.
\end{classdesc}

\begin{methoddesc}[MinimizerLBFGS]{setTolerance}{\optional{x_tol=1e-5} \optional{, J_tol=\None}}
sets the tolerances for the stopping criterions. 
\member{x_tol} sets the tolerance for the unknown level set function~\ref{EQU:MIN:3a}. If \member{x_tol}=\None 
...
If \member{J_tol} = \None
tolerance on cost function is not checked (recommended). If \member{x_tol} and \member{J_tol} are both set
...
the maximum number of iteration is reached \exception{MinimizerMaxIterReached} exception is thrown.
\end{methoddesc}
...
\begin{methoddesc}[MinimizerLBFGS]{setOptions}{\optional{truncation=30} \optional{, restart=60}}
set solver options. \member{truncation} defines number of gradients to keep in memory.
...
\member{m0} set the initial guess.
\end{methoddesc}
...
\section{The L-BFGS Algortihm}


%\section{Limited-Memory BFGS Method}\label{sec:LBFGS}
The L-BFGS algorithm, see~\cite{Nocedal1980}, is as follows:
\begin{algorithm}
    \mbox{L-BFGS}
    \begin{program}
        \BEGIN
