...
In this next example we want to calculate the displacement field $u\hackscore{i}$ for any time $t>0$ by solving the wave equation:\index{wave equation}
...
and $x\hackscore{1}$ direction and height $H$ in $x\hackscore{2}$ direction.
$\rho$ is the known density which may be a function of its location.
$\sigma\hackscore{ij}$ is the stress field\index{stress} which in case of an
...
and $\delta\hackscore{ij}$ denotes the Kronecker symbol\index{Kronecker symbol}.
...
Here we are modelling a point source at the point $x\hackscore C$ in the
$x\hackscore{0}$-direction which is a negative pulse of amplitude
$U\hackscore{0}$ followed by the same positive pulse.
...
\end{eqnarray}
for all $t\ge0$ where $\alpha$ is the width of the pulse and $t\hackscore{0}$
is the time when the pulse changes from negative to positive.
In the simulations we will choose $\alpha=0.3$ and $t\hackscore{0}=2$ (see
\fig{WAVE FIG 1.2}) and apply the source as a constraint in a sphere of small
radius around the point $x\hackscore C$.  
...
$x\hackscore C$ (we will use 3\% of the width of the domain):  
...
of the PDE.
When using lumping the presence of \var{A}, \var{B} or \var{C} will produce
wrong results.
...
(in this case $x\hackscore C$) from functions such as the displacement vector \code{u}.
...
Courant condition\index{Courant condition}\index{explicit scheme!Courant condition}
...
($x\hackscore 0$ and $x\hackscore 1$ directions, i.e. \code{l0} and \code{l1}).
The variable \code{ne} gives the number of elements in $x\hackscore{0}$ and $x\hackscore 1$ directions.
The depth of the block is aligned with the $x\hackscore{2}$-direction.
The depth (\code{l2}) of the block in the $x\hackscore{2}$-direction is chosen
so that there are 10 elements, and the magnitude of the depth is chosen such
that the elements become cubic.
We chose 10 for the number of elements in the $x\hackscore{2}$-direction so
that the computation is faster. \code{Brick($n\hackscore 0,  n\hackscore 1, n\hackscore 2,l\hackscore 0,l\hackscore 1,l\hackscore 2$)}
is an \finley function which creates a rectangular mesh with $n\hackscore 0 \times n\hackscore 1 \times n\hackscore2$
elements over the brick $[0,l\hackscore 0] \times [0,l\hackscore 1] \times [0,l\hackscore 2]$.
\begin{python}
...
Using \function{inf} ensures that the Courant condition~\ref{WAVE dyn 66} holds
...
also to choose the results (acceleration, displacement or $u\hackscore x$) by
...
\begin{python}
  ...
  u_pc_data.close()
\end{python}
The file \code{U_pc.csv} stores 4 columns of data: $t,u\hackscore x,u\hackscore y,u\hackscore z$
respectively, where $u\hackscore x,u\hackscore y,u\hackscore z$ are the
$x\hackscore 0,x\hackscore 1,x\hackscore 2$ components of the displacement
...
Please note that through the \code{getMPIRankWorld() == 0} statement the plot
