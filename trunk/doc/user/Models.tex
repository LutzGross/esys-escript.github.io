...
\begin{methoddesc}[IncompressibleIsotropicFlowCartesian]{setExternals}{\optional{F=None, \optional{f=None, \optional{fixed_v_mask=None, \optional{v_boundary=None}}}}}
assigns values to external forces and boundary conditions. Between two calls only variables with a new values need to be set. 

In any call all values must be set.
\var{f} defines the external force $f$, \var{eta} the viscosity $\eta$,
\var{surface_stress} the surface stress $s$ and \var{stress} the initial stress $\sigma$.
The locations and compontents where the velocity is fixed are set by 
the values of \var{fixed_u_mask}. The method will try to cast the given values to appropriate 
