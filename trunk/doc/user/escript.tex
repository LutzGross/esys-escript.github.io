...
Now consider a 2D example. We will interpolate a surface such that the bottom
...
In the 2D case, the parameters for the \var{x1} direction (min=0, step=0.55)
come first followed by the \var{x0} data object and its parameters.
...
Passing \code{check_boundaries=True} will cause \var{interpolateTable} to
reject any points outside the boundaries.
...
\begin{methoddesc}[DataManager]{getCycle}{}
    returns the export cycle, i.e. the number of time \var{export()} has been
    called.
\end{methoddesc}
