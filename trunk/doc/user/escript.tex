...
\begin{figure}
\includegraphics[width=\textwidth]{EscriptDiagram1}
...
\item \var{ReducedSolution(mydomain)}: solutions of a PDE with a reduced smoothness requirement, eg. using a
lower order approximation on the same element or using macro elements\index{macro elements}.  
...
respectively\index{interpolation}. Interpolation happens automatically in \escript
whenever it is required\index{interpolation}. The user needs to be aware that an
...
\index{projection}\index{interpolation}.
...
\begin{figure}
\includegraphics[width=\textwidth]{EscriptDiagram2}
