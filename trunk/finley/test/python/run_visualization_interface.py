...
class Test_VisualizationInterface(unittest.TestCase):
   ...
   def elementCompareWithMap(self, e1, e2, mappy):
     ...
     for i in range(0, len(vector1)):
       ...
       if int(vector1[i]) >= len(mappy): return False
       if mappy[int(vector1[i])] != int(vector2[i]): return False
     return True

   def compareDataSetWithMap(self, d1, d2, mappy):
     if len(d1) != len(d2): return False
     for i in range(0, len(d1)):
       if mappy[i] < 0: return False
       if mappy[i] >= len(d1): return False
       if not self.numericCompareL2(d1[i], d2[mappy[i]]): return False
     return True
   ...
   def saveVtkCompare(self, file1, file2):
     ...
     for i in range(0, len(nodeList1)):
       ...
       for j in range(len(nodeList2)-1, -1, -1):
         if self.numericCompareL2(resstr, nodeList2[j]):
           nodeMap1to2[i] = j
           nodeList2[j] = None
           break
     ...
     for i in range(0, len(elementList1)):
       resstr = elementList1[i]
       for j in range(len(elementList2)-1, -1, -1):
         if self.elementCompareWithMap(resstr, elementList2[j], nodeMap1to2):
           elementMap1to2[i] = j
           elementList2[j] = None
           break
