...
FloatVectorType::reference
getDRef(FloatVectorType& data,const ShapeType& shape,int i, int j)
{
   return data[getRelIndex(shape,i,j)];
}

FloatVectorType::reference
getDRef(FloatVectorType& data,const ShapeType& shape,int i, int j, int k)
{
...
//   DataArrayView pointData(data,shape);
  ...
  shape2.push_back(3);
  shape2.push_back(3);
  DataTypes::FloatVectorType data2(DataTypes::noValues(shape2),0);
