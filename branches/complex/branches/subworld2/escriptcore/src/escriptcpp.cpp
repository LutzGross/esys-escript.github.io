...
/* begin SubWorld things */
  ...
  def("makeLocalOnly", escript::makeNonReducedVariable, "Create a variable which is not connected to copies in other worlds and is not reduced.");
  ...
  class_<escript::SplitWorld, boost::noncopyable>("SplitWorld", "Manages a group of sub worlds", init<unsigned int>(args("num_worlds")))
    ...
    .def("getNumWorlds", &escript::SplitWorld::getWorldCount);
