...
\footnotetext{The \openmp support in gcc-2.1 is buggy. In particular, the version of gcc-2.1 requires
extra compile options to build escript. Even when built, escript crashes in tests.}
\addtocounter{footnote}{1}
...
A mark means that combination passes our tests.
...
This actually turns out to be pretty simple but see Section~\ref{sec:gccbuilding} for comments.
%Want to make sure that they specify amd64 if machine has more than 4GB of memory.
%This could go in an appendix??
