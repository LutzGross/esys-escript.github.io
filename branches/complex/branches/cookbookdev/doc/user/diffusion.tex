...
The \LinearPDE class provides the method \method{checkSymmetry} to check if the given PDE is symmetric. 
\begin{python}
mypde=LinearPDE(mydomain)
mypde.setValue(A=kappa*kronecker(mydomain),d=eta)
...
print mypde.checkSymmetry() # returns True
\end{python}
Unfortunately, a \method{checkSymmetry} is very expensive and is only recommended only for
