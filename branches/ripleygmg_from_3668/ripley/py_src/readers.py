...
__author__="Lutz Gross, l.gross@uq.edu.au, Joel Fenwick, Cihan Altinay"

from esys.pycad.gmsh import Design as GMSHDesign
from ripleycpp import ReadGmsh

def MakeDomain(design, integrationOrder=-1, reducedIntegrationOrder=-1, \
               optimizeLabeling=True, useMacroElements=False):
    """
    Creates a Ripley `Domain` from a `esys.pycad.design.Design` object.
    ...
    """
    if useMacroElements:
        raise TypeError("Ripley does not support macro elements")
    if isinstance(design, GMSHDesign):
        ...
        raise TypeError("Ripley does not support %s designs."%design.__class__.__name__)
    # fill in the tag map
