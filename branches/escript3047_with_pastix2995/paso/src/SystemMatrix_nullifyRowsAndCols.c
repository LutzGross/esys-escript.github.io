...
/*       if (A->type & MATRIX_FORMAT_CSC) {
         ...
         if (Paso_noError()) {
            ...
            if (A->type & MATRIX_FORMAT_CSC) {
               Paso_SparseMatrix_nullifyRowsAndCols_CSC_BLK1(A->col_coupleBlock,mask_row,remote_values,0.); 
            } else {
            ...
            if (A->type & MATRIX_FORMAT_CSC) {
               Paso_SparseMatrix_nullifyRowsAndCols_CSC_BLK1(A->row_coupleBlock,remote_values,mask_col,0.);
            } else {
...
/*       if (A->type & MATRIX_FORMAT_CSC) {
         ...
         if (Paso_noError()) {
            ...
            remote_values=Paso_SystemMatrix_finishColCollect(A);
            if (A->type & MATRIX_FORMAT_CSC) {
               Paso_SparseMatrix_nullifyRowsAndCols_CSC(A->col_coupleBlock,mask_row,remote_values,0.);
            } else {
               Paso_SparseMatrix_nullifyRowsAndCols_CSR(A->col_coupleBlock,mask_row,remote_values,0.);
            }
            remote_values=Paso_SystemMatrix_finishRowCollect(A);
            if (A->type & MATRIX_FORMAT_CSC) {
               Paso_SparseMatrix_nullifyRowsAndCols_CSC(A->col_coupleBlock,remote_values,mask_col,0.);
            } else {
