...
template <class T>
SystemMatrix_ptr<T> SystemMatrix<T>::loadMM_toCSR(const char *filename)
{
    ...
    out.reset(new SystemMatrix<T>(MATRIX_FORMAT_DEFAULT, pattern, 1, 1, true,
...
template <class T>
SystemMatrix_ptr<T> SystemMatrix<T>::loadMM_toCSC(const char* filename)
{
    ...
    out.reset(new SystemMatrix<T>(MATRIX_FORMAT_CSC, pattern, 1, 1, true,
