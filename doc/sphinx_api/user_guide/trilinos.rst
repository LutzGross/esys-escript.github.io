...
Trilinos has a number of packages and provides a large collection of both direct and indirect solvers. We refer the reader to  for details. Escript needs to be installed with Trilinos to be able to use the Trilinos solvers. See the install guide for details. We show a few examples for the Trilinos options with a simple example.

Consider Laplacian in domain (:math:`\Omega\in \mathbb{R}^3`) with a simple right hand side,
...
with :math:`\mathbf{n}` the outward normal, with :math:`\Gamma_D` the left boundary and :math:`\Gamma=\partial\Omega\backslash\Gamma_D`. Gravity forward weak form of PDE (`[CONST1a] <#CONST1a>`__)-(`[BC2] <#BC2>`__), where :math:`(~,~)` is the standard :math:`L^2` inner product on :math:`\Omega`, is
...
for all admissible potential functions :math:`v`.

For this example, we just consider a simple, unstructured, 3D domain. The mesh is created with GMSH using “simplemesh.geo“ geometry file `[simplemesh] <#simplemesh>`__.

.. code:: python

   [caption=simplemesh.geo,label=simplemesh]
...
to generate the mesh file “simplemesh.msh“.

The most basic escript script to solve this pde using escript defaults is in program `[basic] <#basic>`__. T he computed solution is saved in a silo file "asimple.silo" and can be visualized using *VisIt*.

.. code:: python

   [caption=basic solve using defaults only, label=basic ]
...
This script uses default Trilinos solver PCG with Jacobi preconditioner. (It takes 271 iterations to reach the default tolerance of :math:`10^{-8}`). Tolerance and solver output can be controlled by adding to the basic listing.

.. code:: python

   [caption=tolerance , label=tolerance ]
...
To choose the default Trilinos direct solver, set the tolerance for the PDE solver to :math:`10^{-8}`, use the Trilinos suite of solvers and output information about the solvers, we add code to listing `[basic] <#basic>`__.

.. code:: python

   [caption=default Direct , label=defaultdirect ]
   options = pde.getSolverOptions()  
...
To use SUPERLU the code that needs to be added to listing `[basic] <#basic>`__ is

.. code:: python

   [caption=SuperLU, label=superLU ]
   options = pde.getSolverOptions()  
   options.setTolerance(1e-8)        
   options.setPackage(SolverOptions.TRILINOS)
...
Trilinos is the default solver package so the setPackage line is not really necessary. If Trilions is not installed with SuperLU a direct fall back solver will be chosen.
...
.. code:: python
   ...
   [caption=Preconditioned conjugate gradient defaults, label=PCG]
...
where :math:`\mathbf{u}` is the unknown :math:`n\times 1` vector, :math:`\mathbf{A}` is the :math:`n\times n` matrix and :math:`\mathbf{f}` is the nown right hand side :math:`n\times 1` vector. The residual :math:`\mathbf{r}` is defined
...
where :math:`\mathbf{e}=\mathbf{u}^*-\mathbf{u}` is the error and :math:`\mathbf{u}^*` is the exact solution. Relaxing on `[resEQ] <#resEQ>`__) with the residual as the right hand side is the same as relaxing on (`[matrixEQ] <#matrixEQ>`__) with the original right and side. We use superscripts on these terms to indicate the discretization representing element length, :math:`h`, for a fine grid and :math:`H` for one level coarser discretization, :math:`h<H`. For a structured mesh, :math:`H=\frac{h}{2}` and :math:`\mathbf{A}^h` is an :math:`n\times n` matrix and :math:`\mathbf{A}^H` is an :math:`N\times N` matrix obtained in the coarsening process with :math:`N<n`. Prolongation operator, :math:`\mathbf{P}^h_H`, an :math:`n\times N` matrix, is used to interpolate the error from the coarse grid to the fine grid and restriction operator :math:`\mathbf{P}_h^H`, an :math:`N\times n` matrix, is used to restrict the residual from the fine grid to the coarse grid. It is not necessary for the restriction operator to be the transpose of the interpolation operator. The coarse grid matrix operator :math:`\mathbf{A}^H` is computed by multiplying the fine grid matrix operator :math:`\mathbf{A}^h` on the left by the restriction operator and the right by the interpolation operator resulting in an :math:`N\times N` matrix.

The MG algorithm is best described using a recursion algorithm. After iteration on a fine grid, the fine grid error is smooth and the residual can be restricted to a coarse grid. The error on the coarse grid appears more oscillatory and is (smoothed) reduced by iteration. If this is the coarsest grid then the matrix equation is solved using a direct method or sufficient iterations of the solver to get within discretization error, otherwise the algorithm is called again with the coarse grid replacing the fine grid and the next coarser grid as the coarse grid. The coarse grid error, is interpolated to the fine grid where it corrects the fine grid approximation and post-smoothing iteration smooths the error. For structured grids, the choices for interpolating from a coarse grid to a fine grid or restricting from a fine grid to a coarse grid are reasonably obvious, see .

.. table:: An AMG V(p.q) cycle algorithm to solve :math:`\mathbf{A}\mathbf{u}=\mathbf{f}`, where :math:`h` and :math:`H` represent grid sizes with :math:`h<H`.

   +-------------------------------------------------------------------+--------------------------------------+---+---+
   | AMG(:math:`\mathbf{u}^h;\mathbf{A}^h,\mathbf{f}^h)`               |                                      |   |   |
   +===================================================================+======================================+===+===+
   | :math:`\mathbf{A}^h\mathbf{u}^h=\mathbf{f}^h`                     | **p steps pre-smoothing iteration**  |   |   |
   +-------------------------------------------------------------------+--------------------------------------+---+---+
   | :math:`\mathbf{r}^h=\mathbf{f}^h-\mathbf{A}^h \mathbf{u}^h`       | **fine grid residual**               |   |   |
   +-------------------------------------------------------------------+--------------------------------------+---+---+
   | :math:`\mathbf{r}^H=\mathbf{R}_h^H \mathbf{r}^h`                  | **restrict residual**                |   |   |
   +-------------------------------------------------------------------+--------------------------------------+---+---+
   | :math:`\mathbf{A}^H=\mathbf{R}_h^{H} \mathbf{A}^h \mathbf{P}_H^h` | **restrict operator**                |   |   |
   +-------------------------------------------------------------------+--------------------------------------+---+---+
   | if not coarsest                                                   |                                      |   |   |
   +-------------------------------------------------------------------+--------------------------------------+---+---+
   | AMG(:math:`\mathbf{e}^H;\mathbf{A}^H,\mathbf{r}^H`)               | **recursion**                        |   |   |
   +-------------------------------------------------------------------+--------------------------------------+---+---+
   | else                                                              |                                      |   |   |
   +-------------------------------------------------------------------+--------------------------------------+---+---+
   | :math:`\mathbf{A}^H\mathbf{e}^H=\mathbf{r}^H`                     | **coarsest solve**                   |   |   |
   +-------------------------------------------------------------------+--------------------------------------+---+---+
   | :math:`\mathbf{e}^h=\mathbf{P}_H^h \mathbf{e}^H`                  | **interpolate error**                |   |   |
   +-------------------------------------------------------------------+--------------------------------------+---+---+
   | :math:`\mathbf{u}^h=\mathbf{u}^h+\mathbf{e}^h`                    | **fine grid correction**             |   |   |
   +-------------------------------------------------------------------+--------------------------------------+---+---+
   | :math:`\mathbf{A}^h\mathbf{u}^h=\mathbf{f}^h`                     | **q steps post-smoothing iteration** |   |   |
   +-------------------------------------------------------------------+--------------------------------------+---+---+
   | return :math:`\mathbf{u}^h`                                       |                                      |   |   |
   +-------------------------------------------------------------------+--------------------------------------+---+---+

Algebraic multigrid methods were developed for unstructured grids and do not reference the grid but instead use interpolation and restriction operators derived from the matrix (see ). The terms "coarse grid" and "fine grid" are still used but do not refer to actual grids. "Smooth error" is defined to be the error not reduced by iteration and "oscillatory error" is the error reduced by iteration. Coarse levels are chosen from the relative sizes of the off diagonal terms in the fine matrix. Once the coarse grid is chosen the restriction and interpolation operators are computed. Restriction operators need to be chosen so that "smooth error" on a fine grid will appear "oscillatory" on a coarse grid ensuring that it can be reduced by iterating on this grid. There are a number of algorithm options available in Trilinos to compute the coarse grid and the choice will depend on the original PDE and smoothing options. For any multigrid method, there are basic choices:
...
Default MueLu Trilinos options for algebraic multigrid preconditioned conjugate gradient (AMG-PCG)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

More detail on the various options can be found in the MUELU user guide and other Trilinos user guides . MUELU uses other Trilinos packages and to access these parameters the XML file must be used. Recall :math:`\mathbf{R}_h^H` is the restriction operator and :math:`\mathbf{P}^h_H` is the interpolation (prolongation) operator. The default values used are shown in Table `[defaultAMG] <#defaultAMG>`__. To use AMG-PCG with default paramerters we use script `[basicAMG] <#basicAMG>`__.

.. table:: Default parameters for AMG-PCG

   +---------------------------------+---------------------------------------------------+
   | number of equations             | 1                                                 |
   +---------------------------------+---------------------------------------------------+
   | problem: symmetric              | True, :math:`\mathbf{R}_h^H = (\mathbf{P}^h_H)^t` |
   +---------------------------------+---------------------------------------------------+
...
.. code:: python
   ...
   [caption=basic Trilinos PCG-AMG defaults only script, label=basicAMG ]
   from esys.escript import *
   from esys.weipa import saveVTK, saveSilo
   from esys.escript.linearPDEs import LinearSinglePDE
   from esys.finley import ReadGmsh
...
Maximum levels, coarse mesh, coarse solver
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
Relationship between :math:`\mathbf{R}_h^H` and :math:`\mathbf{P}^h_H`
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

For :math:`\mathbf{R}_h^H=(\mathbf{P}^h_H)^t`
...
A very simple XML file is in file `[simplebob] <#simplebob>`__

.. code:: python

   [caption=simplebob.xml,label=simplebob]
...
A more complicated example that controls the number of pre and post sweeps is in the listing `[complicatedbob] <#complicatedbob>`__

.. _complicatedbob:

.. code-block:: xml
   :caption: complicatedbob.xml
