...
Currently \finley supports \member{SolverOptions.TRILINOS} (default when available),
\member{SolverOptions.PASO}, \member{SolverOptions.MKL}\footnote{If the stiffness matrix is
...
\PASO built-in solver library, see \Sec{chap:finley}.
When Trilinos is not available, \PASO is used as the default package.
\end{memberdesc}
