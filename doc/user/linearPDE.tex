...
\subsubsection*{class LinearPDE(domain,numEquations=0,numSolutions=0)}
...
\paragraph{LinearPDE.setValue(\optional{A)}
\optional{, B},
...
\paragraph{LinearPDE.setSolverOptions(\optional{options=None)}
}
...
\subsubsection*{class Poisson(domain)}
opens a Poisson equation on the \Domain domain. \Poisson is derived from \LinearPDE.

\paragraph{Poisson.setValue(f=escript.Data(),q=escript.Data())}
...
\subsubsection*{class Helmholtz(domain)}
opens a Helmholtz equation on the \Domain domain. \Helmholtz is derived from \LinearPDE.

\paragraph{Helmholtz.setValue(\optional{omega)}
\optional{, k} \optional{, f} \optional{, alpha} \optional{, g} \optional{, r} \optional{, q}}
...
\subsubsection*{class Lame(domain)}
opens a Lam\'e equation on the \Domain domain. \Lame is derived from \LinearPDE.

\paragraph{Lame.setValue(\optional{lame_lambda)}
\optional{, lame_mu} \optional{, F} \optional{, sigma} \optional{, f} \optional{, r} \optional{, q}}
...
\subsubsection*{class Projector(domain\optional{, reduce=\True \optional{, fast=\True)}
}}
...
\paragraph{Projector.getSolverOptions()}
returns the solver options for solving the PDE. In fact, the method returns
a \SolverOptions class object which can be used to modify the tolerance,
the solver or the preconditioner, see \Sec{SEC Solver Options} for details.


\paragraph{Projector.getValue(input_data)}
...
\subsubsection*{class SolverOptions}
...
\paragraph{SolverOptions.getSummary()}
returns a string reporting the current settings.


\paragraph{SolverOptions.getName(key)}
returns the name as a string of a given key.


\paragraph{SolverOptions.setSolverMethod(method)}
...
The value of \var{method} must be one of the constants:\\
 \member{SolverOptions.DEFAULT} -- use default solver depending on other options: for real PDEs, \PCG is used for symmetric problems and \GMRES otherwise; for complex PDEs, \BiCGStab is used (PCG and GMRES are not supported for complex scalar types with the Trilinos Belos library)\\
...
\paragraph{SolverOptions.getSolverMethod()}
returns the key of the solver method to be used.


\paragraph{SolverOptions.setPreconditioner(preconditioner)}
...
\paragraph{SolverOptions.getPreconditioner()}
returns the key of the preconditioner to be used.


\paragraph{SolverOptions.setPackage(package)}
selects a third-party direct solver sub-library.
The value of \var{package} must be one of the constants:\\
 ...
 \member{SolverOptions.MUMPS} -- MUMPS parallel direct solver\\
 \member{SolverOptions.UMFPACK} -- direct solver from the UMFPACK library.\\
Not all packages are supported on all implementations.  An exception is thrown
if a requested package was not compiled in.
Note: to select the \emph{solver framework} (Trilinos or Paso) use
\member{SolverFramework} when creating the domain
(see \member{SolverFramework.trilinos()} and \member{SolverFramework.paso()}).
...
\paragraph{SolverOptions.resetDiagnostics(\optional{all=False)}
}
resets the diagnostics. If \var{all} is \True all diagnostics, including
accumulative counters, are reset.


\paragraph{SolverOptions.getDiagnostics(\optional{ name)}
}
...
\paragraph{SolverOptions.hasConverged()}
...
\paragraph{SolverOptions.setReordering(ordering)}
...
\paragraph{SolverOptions.getReordering()}
returns the key of the reordering method to be applied if supported by the solver.


\paragraph{SolverOptions.setRestart(\optional{restart=None)}
}
...
\paragraph{SolverOptions.getRestart()}
returns the number of iterations steps after which \GMRES performs a restart.


\paragraph{SolverOptions.setTruncation(\optional{truncation=20)}
}
...
\paragraph{SolverOptions.getTruncation()}
returns the number of residuals in \GMRES to be stored for orthogonalization.



\paragraph{SolverOptions.setIterMax(\optional{iter_max=10000)}
}
sets the maximum number of iteration steps.


\paragraph{SolverOptions.getIterMax()}
returns maximum number of iteration steps.


%\paragraph{SolverOptions.setLevelMax(\optional{level_max=10)}
%}
...
\paragraph{SolverOptions.getTolerance()}
returns the relative tolerance for the solver.


\paragraph{SolverOptions.setAbsoluteTolerance(\optional{atol=0.)}
}
...
\paragraph{SolverOptions.setInnerToleranceAdaptionOn()}
switches the automatic selection of inner tolerance on.


\paragraph{SolverOptions.setInnerToleranceAdaptionOff()}
switches the automatic selection of inner tolerance off.


\paragraph{SolverOptions.setInnerIterMax(\optional{iter_max=10)}
}
sets the maximum number of iteration steps for the inner iteration.


\paragraph{SolverOptions.getInnerIterMax()}
returns the maximum number of inner iteration steps.


\paragraph{SolverOptions.acceptConvergenceFailure()}
...
\paragraph{SolverOptions.setAcceptanceConvergenceFailureOn()}
switches the acceptance of a failure of convergence on.


\paragraph{SolverOptions.setAcceptanceConvergenceFailureOff()}
...
For real PDEs, \PCG is used for symmetric problems and \BiCGStab otherwise, both with
\JACOBI preconditioner.
For complex PDEs with Trilinos, \BiCGStab is always used as the default
(PCG and GMRES are not supported for complex scalar types in the Belos solver library).
\end{memberdesc}
...
\label{SEC Solver Framework}

\subsubsection*{class SolverFramework}
\class{SolverFramework} selects the underlying solver backend (Trilinos or Paso)
used by a domain.  It is passed to the domain constructor via the
\var{framework} keyword argument:
\begin{python}
from esys.escript import SolverFramework
from esys.finley import ReadGmsh
domain = ReadGmsh("mesh.msh", 3, optimize=True,
                  framework=SolverFramework.trilinos())
\end{python}
Once set, the framework is fixed for the lifetime of the domain.
The solver method and preconditioner are still controlled through
\class{SolverOptions} as before.

\paragraph{SolverFramework.trilinos()}
returns a \class{SolverFramework} object for the Trilinos backend.
Raises \exception{RuntimeError} if Trilinos was not compiled in.

\paragraph{SolverFramework.paso()}
returns a \class{SolverFramework} object for the built-in Paso backend.
Raises \exception{RuntimeError} if Paso was not compiled in.

\paragraph{SolverFramework.getDefault()}
returns the process-wide default framework.
When both backends are compiled in, Trilinos is the default.

\paragraph{SolverFramework.isAvailable(type)}
returns \True if the framework identified by \var{type}
(\member{SolverFramework.PASO} or \member{SolverFramework.TRILINOS}) was
compiled in.

\begin{memberdesc}[SolverFramework]{PASO}
integer constant identifying the \PASO backend.
\end{memberdesc}

\begin{memberdesc}[SolverFramework]{TRILINOS}
integer constant identifying the Trilinos backend, see Chapter~\ref{TRILINOS}.
\end{memberdesc}
