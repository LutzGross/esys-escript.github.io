...
\item[\programopt{-b}] do not invoke python. This is used to run non-python programs.
 ...
 \item[\programopt{-o}] switches on the redirection of output of processors with \MPI rank greater than zero to the files \file{stdout_\var{r}.out} and \file{stderr_\var{r}.out} where \var{r} is the rank of the processor. The option overwrites the value of the \env{ESCRIPT_CREATESTDFILES} environment variable 
...
If the \programopt{-o} or \env{ESCRIPT_CREATESTDFILES} is set\footnote{That is, it has a non-empty value.}, then the standard and error output from any process other than the master process will be written to files of the names \file{stdout_\var{r}.out} and \file{stderr_\var{r}.out} (where 
