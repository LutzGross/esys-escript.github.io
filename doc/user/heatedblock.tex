...
introduced in \Sec{WAVE CHAP} but the inertia term $\rho u\hackscore{i,tt}$
...
of linear PDEs for the displacements $u$ which is called the Lame equation\index{Lame equation}.
...
Analogously to the concept of symmetry for a single PDE, we call the PDE
...
Typically the von-Mises stress\index{von-Mises stress} defined by
\begin{equation}
...
The following script, which is available in \file{heatedbox.py} in the
