...
When Trilinos is compiled in it is used as the default backend, so no explicit \class{SolverFramework} is needed here.
To select Trilinos explicitly (or to force it when multiple backends are available), pass
\code{framework=SolverFramework.trilinos()} to the domain constructor.
This script uses the default Trilinos solver PCG with Jacobi preconditioner for real symmetric PDEs.
(It takes 271 iterations to reach the default tolerance of $10^{-8}$).
The default solver selection depends on the PDE type:
\begin{itemize}
  \item \textbf{Real symmetric PDEs}: PCG with Jacobi preconditioner
  \item \textbf{Real non-symmetric PDEs}: GMRES with Jacobi preconditioner
  \item \textbf{Complex PDEs} (\verb|isComplex=True|): BiCGStab with Jacobi preconditioner
    (PCG and GMRES are not supported for complex scalar types in Belos)
\end{itemize}
...
\hline
PCG & \verb|SolverOptions.PCG| & Preconditioned Conjugate Gradient (real PDEs only) \\
GMRES & \verb|SolverOptions.GMRES| & Generalized Minimal Residual (real PDEs only) \\
BiCGStab & \verb|SolverOptions.BICGSTAB| & BiConjugate Gradient Stabilized (real and complex) \\
TFQMR & \verb|SolverOptions.TFQMR| & Transpose-Free QMR (real and complex) \\
...
options.setVerbosityOn()
\end{python}
If Trilinos is not installed with SuperLU a direct fall back solver will be chosen.
...
\item For \textbf{complex-valued PDEs} (\verb|isComplex=True|), use \textbf{BiCGStab} or \textbf{TFQMR}.
  The default solver is automatically set to BiCGStab for complex PDEs.
  PCG and GMRES are not supported for complex scalar types in the Belos solver library
  (PCG is not designed for complex Hermitian systems, and GMRES has a known upstream bug
...
\item \textbf{PCG or GMRES fails for complex PDEs}: Belos PCG (CG) is not designed for complex Hermitian
  systems, and Belos GMRES has a known upstream bug with complex scalar types. For complex PDEs,
  use \verb|SolverOptions.BICGSTAB| or \verb|SolverOptions.TFQMR| instead. The default solver
  is automatically set to BiCGStab when \verb|isComplex=True|.
