...
\subsection*{6.0}
\begin{itemize}
\item escript now uses Trilinos 14
\item escript now has a built-in Trilinos 14 compiler
\item escript now utilises the p4est library
\item The dudley module has been removed
\item The oxley module has been added
\item sympy has been removed
\end{itemize}

\subsection*{5.9}
\begin{itemize}
\item escript is now compatible with OPENMPI 4.*
\item escript is now compatible with Trilinos 13.*
\item escript now uses scipy.interpolate.griddata instead of the deprecated matplotlib.mlab.griddata
\item Updated API
\item Automated unit testing using github
...
\item Finley can now read gmsh MSH 4.0 and MSH 4.1 files correctly, including tag information.
...
\item Finley now allows users to interpolate from Reduced Function Data to Function Data.
...
\item \finley now supports GMSH version 4.0 MSH files
...
This does not change the value itself, (eg $7 \rightarrow 7+0j$) but matters for operations like \emph{sqrt} which behave differently with complex inputs.
  ...
  See \texttt{scons/templates/options.md} for the options.
...
\begin{itemize}
 ...
 \item setX on \finley domains will now only accept coordinates from ContinuousFunction spaces.
