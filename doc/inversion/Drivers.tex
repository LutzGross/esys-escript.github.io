...
template for inversion drivers. By default the limited-memory Broyden-Fletcher-Goldfarb-Shanno (\emph{L-BFGS})~\cite{Nocedal1980}\index{L-BFGS} solver is used.
\end{classdesc*}
...
if the relative change of the level set function is less than or equal \member{tol}. 
If \member{atol} is set the iteration is terminated if the change of the cost
function relative to the initial value is less than or equal \member{atol}.
If both tolerances are set both stopping criteria need to be met.
By default \member{tol}=1e-4 and \member{atol}=\None.
\end{methoddesc}
