...
In practice the magnetic flux density $b^{(s)}$ is measured along a certain direction $d^{(s)}_i$ with a 
...
\optional{, scale=1
\optional{, tol=1e-8}}}}
opens a magnetic forward model over the \Domain \var{domain} with 
weighting factors \var{w} ($=\omega^{(s)}$) and measured magnetic flux density anomalies \var{B} ($=B^{(s)}$).
The weighting factors and the  measured magnetic flux density anomalies must be vectors.
\var{background_field} defines the background magnetic flux density $B^b$. 
\var{background_field} must be a vector.
If \var{useSphericalCoordinates} is \True spherical coordinates are used. 
\var{scale} sets the scaling factor $\alpha$. 
\var{tol} set the tolerance for the solution of the PDE~(\ref{ref:MAG:EQU:8}).
