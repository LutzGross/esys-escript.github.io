...
and our signed distance from the the block-block interface $x$.
...
the  \verb|Function(domain).getX()| returns the coordinates of numerical
...
ContinuousFunction()  can be transformed into a \verb|FunctionOnBoundary()|
or \verb|Function()|. On the other hand there is not enough information in a
...
\verb|ndx| would be 1 to 10\% of the length. Our \verb|ndy| need only be 1, this
...
\verb|whereNegative| returns the value $1$ at those sample points where the
argument 
has a negative value. Otherwise zero is returned. If \verb|x| are the
$x\hackscore{0}$ 
...
On most operating systems\footnote{The you can use \texttt{run-escript} launcher
...
\end{python}
Here we are not using the \verb|from pylab import *| in order to avoid name
...
result into the file \verb|totE.png| which can be displayed by (almost) any
image viewer. 
...
\verb|save_path| is name of directory we want to put the results the command
is; 
\begin{python}
import os
...
Prior to the \verb|while| loop we will need to extract our finite solution
