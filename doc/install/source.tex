...
If you are using any version of Linux released in the past few years, then your
system compiler will support \openmp with no extra work (and give better
performance); so you should use it. MSVC 2019 also has \openmp support on
Windows (\openmp 2.0). You will not need MPI unless your computer is some form
of cluster. MPI is not recommended on Windows as it will interfer with Jupyter.
...
\begin{enumerate}
\item Download the Microsoft Visual Studio Community 2019 installer from
\begin{itemize}
\item[] \url{https://visualstudio.microsoft.com}.
\end{itemize}
\item Launch the Visual Studio installer, selecting Individual components:
\begin{itemize}
\item \textbf{MSVC v142 - VS 2019 C++ x64/x86 build tools}
...
\item Gmsh
\begin{enumerate}
\item Download the Gmsh 4.6.0 for Windows 64-bit package from
\begin{itemize}
\item[] \url{http://gmsh.info}.
\end{itemize}
\item Extract the Gmsh package to \verb!C:\Users\%USERNAME%!.
\end{enumerate}
\end{itemize}
...
conda create --name escript
conda deactivate
C:\Users\%USERNAME%\Anaconda3\Scripts\activate
    C:\Users\%USERNAME%\Anaconda3\envs\escript
\end{shellCode}
\item Install required conda modules
\begin{shellCode}
conda install numpy==1.15.4 matplotlib==2.2.2 sympy==1.1.1
    boost gdal git pyproj scons scipy
conda install mumps -c conda-forge
\end{shellCode}
\end{enumerate}
\item Vcpkg packages
\begin{enumerate}
...
bootstrap-vcpkg
\end{shellCode}
\item Install the CppUnit and NetCDF vcpkg packages
\begin{shellCode}
vcpkg install cppunit:x64-windows netcdf-cxx4:x64-windows
\end{shellCode}
\end{enumerate}
\end{itemize}
...
\end{shellCode}
\item Add Gmsh and CppUnit to the Windows System Path.
\begin{shellCode}
set PATH=%PATH%;C:\Users\%USERNAME%\gmsh-4.6.0-Windows64
...
\item Kick-off the \escript build.
\begin{shellCode}
cd C:\Users\%USERNAME%\escript\src
scons -j1 build_full options_file=scons/templates/windows_msvc142_options.py
\end{shellCode}
