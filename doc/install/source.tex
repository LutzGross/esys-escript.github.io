...
Both of those systems require the XCode command line tools to be installed\footnote{As of OSX10.9, the 
...
The \texttt{_options.py} files are located in the scons/templates directory. We suggest that the file most relavent to your os 
...
These instructions were prepared using centos release $6.5$.
...
In the escript source directory:
\begin{shellCode}
scons -j1 options_file=scons/templates/centos6.5_options.py
\end{shellCode}
...
In the escript source directory:
\begin{shellCode}
scons -j1 options_file=scons/templates/fedora18_options.py
\end{shellCode}

\noindent If you wish to test your build, you can use the following:
\begin{shellCode}
scons -j1 py_tests options_file=scons/templates/fedora18_options.py 
\end{shellCode}
...
\subsubsection{Release 9.1}
The following passes the majority of tests but there is an issue related to
some features in the inversion library.
The list of installations ``works'' but is not guaranteed to be
minimal\footnote{Depending on your needs you might be able to get by with a
...
In this case we have three prepared in the \texttt{scons/templates} directory:
\begin{itemize}
