...
"""
RefinementFactory2D / RefinementFactory3D.
...
and nothing in the type system said so. A factory instead collects the
...
from esys.oxley import Rectangle, Brick, RefinementFactory2D, RefinementFactory3D
...
class Test_RefinementFactory2D(unittest.TestCase):
    def setUp(self):
    ...
    def test_uniform_matches_the_constructor(self):
        ...
        for level in (1, 2):
            ...
            f.refineUniform()
            viaFactory = numElements(f.apply(self.domain))
            viaCtor = numElements(Rectangle(n0=N0, n1=N1, l0=L0, l1=L1,
                                            refine_level=level))
            self.assertEqual(viaFactory, viaCtor,
                             "level %d: factory gives %d elements, the "
                             "constructor %d" % (level, viaFactory, viaCtor))
    ...
    def test_level_is_absolute(self):
        ...
        re-applying the same factory changes nothing, and only asking for a
    ...
    def test_empty_factory_changes_nothing(self):
        f = RefinementFactory2D()
...
class Test_RefinementFactory3D(unittest.TestCase):
    def setUp(self):
