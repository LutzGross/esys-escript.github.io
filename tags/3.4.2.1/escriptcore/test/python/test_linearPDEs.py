...
class Test_LinearPDE_noLumping(Test_linearPDEs):
    ...
    def test_SolverOptions(self):
        sb=SolverBuddy()
        so=SolverOptions
        self.assertTrue(sb.getSmoother() == 28, "initial Smoother is wrong.")
        self.assertRaises(ValueError,sb.setSmoother,-1)
        sb.setSmoother(so.GAUSS_SEIDEL)
        self.assertTrue(sb.getSmoother() == 28, "Gauss-Seidel smoother is not set.")
        sb.setSmoother(so.JACOBI)
        self.assertTrue(sb.getSmoother() == 10, "Jacobi smoother is not set.")
        ...
        self.assertTrue(sb.getNumSweeps() == 1, "initial  Sweeps is wrong.")
        ...
        self.assertTrue(sb.getReordering() == 30, "initial Reordering is wrong.")
        self.assertRaises(ValueError,sb.setReordering,-1)
        sb.setReordering(so.NO_REORDERING)
        self.assertTrue(sb.getReordering() == 17, "NO_REORDERING is not set.")
        sb.setReordering(so.MINIMUM_FILL_IN)
        self.assertTrue(sb.getReordering() == 18, "MINIMUM_FILL_IN is not set.")
        sb.setReordering(so.NESTED_DISSECTION)
        self.assertTrue(sb.getReordering() == 19, "NESTED_DISSECTION is not set.")
        sb.setReordering(so.DEFAULT_REORDERING)
        self.assertTrue(sb.getReordering() == 30, "DEFAULT_REORDERING is not set.")
        
        self.assertTrue(sb.getPackage() == 0, "initial solver package is wrong.")
        self.assertRaises(ValueError,sb.setPackage,-1)
        sb.setPackage(so.PASO)
        self.assertTrue(sb.getPackage() == 21, "PASO is not set.")
        sb.setPackage(so.SUPER_LU)
        self.assertTrue(sb.getPackage() == 31, "SUPER_LU is not set.")
        sb.setPackage(so.PASTIX)
        self.assertTrue(sb.getPackage() == 32, "PASTIX is not set.")
        sb.setPackage(so.MKL)
        self.assertTrue(sb.getPackage() == 15, "MKL is not set.")
        sb.setPackage(so.UMFPACK)
        self.assertTrue(sb.getPackage() == 16, "UMFPACK is not set.")
        sb.setPackage(so.TRILINOS)
        self.assertTrue(sb.getPackage() == 24, "TRILINOS is not set.")

        self.assertTrue(sb.getSolverMethod() == 0, "initial SolverMethod is wrong.")
        self.assertRaises(ValueError,sb.setSolverMethod,-1)
        sb.setSolverMethod(so.DIRECT)
        self.assertTrue(sb.getSolverMethod() == 1, "DIRECT is not set.")
        sb.setSolverMethod(so.CHOLEVSKY)
        self.assertTrue(sb.getSolverMethod() == 2, "CHOLEVSKY is not set.")
        sb.setSolverMethod(so.PCG)
        self.assertTrue(sb.getSolverMethod() == 3, "PCG is not set.")
        sb.setSolverMethod(so.CR)
        self.assertTrue(sb.getSolverMethod() == 4, "CR is not set.")
        sb.setSolverMethod(so.CGS)
        self.assertTrue(sb.getSolverMethod() == 5, "CGS is not set.")
        sb.setSolverMethod(so.BICGSTAB)
        self.assertTrue(sb.getSolverMethod() == 6, "BICGSTAB is not set.")
        sb.setSolverMethod(so.GMRES)
        self.assertTrue(sb.getSolverMethod() == 11, "GMRES is not set.")
        sb.setSolverMethod(so.PRES20)
        self.assertTrue(sb.getSolverMethod() == 12, "PRES20 is not set.")
        sb.setSolverMethod(so.LUMPING)
        self.assertTrue(sb.getSolverMethod() == 13, "LUMPING is not set.")
        sb.setSolverMethod(so.ITERATIVE)
        self.assertTrue(sb.getSolverMethod() == 20, "ITERATIVE is not set.")
        sb.setSolverMethod(so.NONLINEAR_GMRES)
        self.assertTrue(sb.getSolverMethod() == 25, "NONLINEAR_GMRES is not set.")
        sb.setSolverMethod(so.TFQMR)
        self.assertTrue(sb.getSolverMethod() == 26, "TFQMR is not set.")
        sb.setSolverMethod(so.MINRES)
        self.assertTrue(sb.getSolverMethod() == 27, "MINRES is not set.")
        sb.setSolverMethod(so.DEFAULT)
        self.assertTrue(sb.getSolverMethod() == 0, "DEFAULT is not set.")

        self.assertTrue(sb.getPreconditioner() == 10, "initial Preconditioner is wrong.")
        self.assertRaises(ValueError,sb.setPreconditioner,-1)
        sb.setPreconditioner(so.ILU0)
        self.assertTrue(sb.getPreconditioner() == 8, "ILU0 is not set.")
        sb.setPreconditioner(so.ILUT)
        self.assertTrue(sb.getPreconditioner() == 9, "ILUT is not set.")
        sb.setPreconditioner(so.JACOBI)
        self.assertTrue(sb.getPreconditioner() == 10, "JACOBI is not set.")
        if getEscriptParamInt('DISABLE_AMG', 0):
            print("AMG test disabled on MPI build")
        else:
            sb.setPreconditioner(so.AMG)
            self.assertTrue(sb.getPreconditioner() == 22, "AMG is not set.")
        sb.setPreconditioner(so.REC_ILU)
        self.assertTrue(sb.getPreconditioner() == 23, "REC_ILU is not set.")
        sb.setPreconditioner(so.GAUSS_SEIDEL)
        self.assertTrue(sb.getPreconditioner() == 28, "GAUSS_SEIDEL is not set.")
        sb.setPreconditioner(so.RILU)
        self.assertTrue(sb.getPreconditioner() == 29, "RILU is not set.")
        sb.setPreconditioner(so.AMLI)
        self.assertTrue(sb.getPreconditioner() == 38, "AMLI is not set.")
        sb.setPreconditioner(so.NO_PRECONDITIONER)
        self.assertTrue(sb.getPreconditioner() == 36, "NO_PRECONDITIONER is not set.")        

        self.assertTrue(sb.getCoarsening() == 0, "initial Coarseningr is wrong.")
        self.assertRaises(ValueError,sb.setCoarsening,-1)
        sb.setCoarsening(so.YAIR_SHAPIRA_COARSENING)
        self.assertTrue(sb.getCoarsening() == 33, "YAIR_SHAPIRA_COARSENING is not set.")
        sb.setCoarsening(so.RUGE_STUEBEN_COARSENING)
        self.assertTrue(sb.getCoarsening() == 34, "RUGE_STUEBEN_COARSENING is not set.")
        sb.setCoarsening(so.AGGREGATION_COARSENING)
        self.assertTrue(sb.getCoarsening() == 35, "AGREGATION_COARSENING is not set.")
        sb.setCoarsening(so.STANDARD_COARSENING)
        self.assertTrue(sb.getCoarsening() == 39, "STANDARD_COARSENING is not set.")
        sb.setCoarsening(so.DEFAULT)
        self.assertTrue(sb.getCoarsening() == 0, "DEFAULT is not set.")
