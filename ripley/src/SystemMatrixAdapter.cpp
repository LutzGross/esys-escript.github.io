...
{
    throw RipleyException("Error - Illegal to generate default SystemMatrixAdapter.");
}
...
void ripley::SystemMatrixAdapter::Print_Matrix_Info(const bool full=false) const
{
    Paso_SystemMatrix* mat=m_system_matrix.get();
...
{
    if( fileName.size() == 0 ) {
        throw RipleyException("Null file name!");
    }
