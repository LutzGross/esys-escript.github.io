...
A collection of general-purpose optimization routines using numpyeric
...
def rosen_der(x):
    ...
    xm_m1 = x[:-2]
    xm_p1 = x[2:]
    der = numpy.zeros(x.shape,x.typecode())
...
def rosen3_hess_p(x,p):
    assert(len(x)==3)
    assert(len(p)==3)
    hessp = numpy.zeros((3,),x.typecode())
...
def rosen3_hess(x):
    assert(len(x)==3)
    hessp = numpy.zeros((3,3),x.typecode())
...
def fmin(func, x0, args=(), xtol=1e-4, ftol=1e-4, maxiter=None, maxfun=None, fulloutput=0, printmessg=1):
    ...
    sim = numpy.zeros((N+1,N),x0.typecode())
    ...
    while (funcalls < maxfun and iterations < maxiter):
        if (max(numpy.ravel(numpy.absolute(sim[1:]-sim[0]))) <= xtol \
            and max(numpy.absolute(fsim[0]-fsim[1:])) <= ftol):
            break
...
def line_search(f, fprime, xk, pk, gfk, args=(), c1=1e-4, c2=0.9, amax=50):
    ...
    Wright and Nocedal in 'numpyerical Optimization', 1999, pg. 59-60
    """
    ...
    derphi_a0 = numpy.dot(apply(fprime,(xk+alpha0*pk,)+args),pk)
    gc = gc + 1

    # check to see if alpha0 = 1 satisfies Strong Wolfe conditions.
    if (phi_a0 <= phi0 + c1*alpha0*derphi0) \
       and (numpy.absolute(derphi_a0) <= c2*numpy.absolute(derphi0)):
        return alpha0, fc, gc
    ...
    while 1:
        ...
        derphi_a1 = numpy.dot(apply(fprime,(xk+alpha1*pk,)+args),pk)
        gc = gc + 1
        if (numpy.absolute(derphi_a1) <= -c2*derphi0):
            return alpha1
...
def line_search_BFGS(f, xk, pk, gfk, args=(), c1=1e-4, alpha0=1):
    ...
    Wright and Nocedal in 'numpyerical Optimization', 1999, pg. 56-57
    """
    ...
    while 1:       # we are assuming pk is a descent direction
        ...
        alpha2 = (-b + numpy.sqrt(numpy.absolute(b**2 - 3 * a * derphi0))) / (3.0*a)
...
def fminBFGS(f, x0, fprime=None, args=(), avegtol=1e-5, maxiter=None, fulloutput=0, printmessg=1):
    ...
    See Wright, and Nocedal 'numpyerical Optimization', 1999, pg. 198.
    """
    ...
    xk = x0
    sk = [2*gtol]
    while (numpy.add.reduce(numpy.absolute(gfk)) > gtol) and (k < maxiter):
        pk = -numpy.dot(Hk,gfk)
...
def fminNCG(f, x0, fprime, fhess_p=None, fhess=None, args=(), avextol=1e-5, maxiter=None, fulloutput=0, printmessg=1):
    ...
    compute it. See Wright, and Nocedal 'numpyerical Optimization', 1999,
    pg. 140.
    """
    ...
    xk = x0
    k = 0
    while (numpy.add.reduce(numpy.absolute(update)) > xtol) and (k < maxiter):
        # Compute a search direction pk by applying the CG method to
        #  del2 f(xk) p = - grad f(xk) starting from 0.
        b = -apply(fprime,(xk,)+args)
        gcalls = gcalls + 1
        maggrad = numpy.add.reduce(numpy.absolute(b))
        eta = min([0.5,numpy.sqrt(maggrad)])
        termcond = eta * maggrad
        xsupi = numpy.zeros((len(x0),))
        ri = -b
        psupi = -ri
        i = 0
        ...
        while numpy.add.reduce(numpy.absolute(ri)) > termcond:
            if fhess is None:
                if approx_hessp:
                ...
                Ap = numpy.matrixmultiply(A,psupi)
            # check curvature
