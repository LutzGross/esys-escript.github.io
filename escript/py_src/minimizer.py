...
def line_search(f, x, p, g_Jx, Jx, args_x, alpha=1.0, alpha_truncationax=50.0,
    ...
    phi_a = phi0
    i = 1
    if (interpolationOrder != 1) and (interpolationOrder != 2) and (interpolationOrder != 3) and (
    ...
    while i < max(IMAX, 2) and alpha > 0.:
        ...
        if (phi_a > phi0 + c1 * alpha * gphi0) or ((phi_a >= old_phi_a) and (i > 1)):
            alpha, phi_a = _zoom(phi, gradphi, phiargs, old_alpha, alpha, old_phi_a, phi_a, c1, c2, phi0, gphi0,
                                 interpolationOrder, tol_df, tol_sm, IMAX=IMAX_ZOOM)
            break
        ...
        if np.abs(gphi_a) <= -c2 * gphi0:
            break
        if gphi_a >= 0:
            alpha, phi_a = _zoom(phi, gradphi, phiargs, alpha, old_alpha, phi_a, old_phi_a, c1, c2, phi0, gphi0,
                                 interpolationOrder, tol_df, tol_sm, IMAX=IMAX_ZOOM)
            break
...
class MinimizerLBFGS(AbstractMinimizer):
    ...
    def run(self, x):
        ...
        # equivalent to getValue() for CostFunctions
        ...
        while not converged and not non_curable_break_down and n_iter < self._imax:
            ...
            while not converged and not break_down and k < self._restart and n_iter < self._imax:
                ...
                if invH_scale is None:
                    ...
                    alpha = 1.0 * alpha
                alpha, Jx_new, g_Jx_new, args_new = line_search(self.getCostFunction(), x, p, g_Jx, Jx, args, alpha,
                                                                interpolationOrder=self.interpolationOrder,
                                                                IMAX=self._max_linesearch_steps,
                                                                IMAX_ZOOM=self._max_zoom_steps)
                ...
                if not break_down:
                    ...
                    if self.getCostFunction().provides_inverse_Hessian_approximation:
                        ...
                        if denom > 0:
                            ...
                            invH_scale = 1. / self._initial_H
                            self.logger.debug("** Break down in H update. Resetting to initial value %s." % (
                                            1. / self._initial_H))
